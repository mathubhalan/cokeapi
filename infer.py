# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 19:02:09 2019

@author: Mathu_Gopalan
"""

import numpy as np
import tensorflow as tf
import cv2 as cv
import json
import glob

class InferImage:
    '''
    tf session and functionlity to infer the image are defined in this class
    '''
    def __init__(self):
        '''
        initilize the InferImage class
        '''
        with tf.gfile.FastGFile('/home/admin/adcontext/finetuned_model/frozen_inference_graph.pb', 'rb') as f:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read())
            print("Init complete")
    
    def infer(self,image):
        '''
        Infer the Image with the pretained tf model
        Input: Image Path
        Output: Returns the predicted class and other details
        '''
        with tf.Session() as sess:
            # Restore session
            sess.graph.as_default()
            tf.import_graph_def(graph_def, name='')
            image_path = self.image
            print(image_path)
            img = cv.imread(image_path)
            out_image_path="/home/admin/adcontext/training_demo/images/CokeAccuracyResults/**/"
            rows = img.shape[0]
            cols = img.shape[1]
            inp = cv.resize(img, (300, 300))
            # BGR2RGB
            inp = inp[:, :, [2, 1, 0]]  
            # Run the model
            out = sess.run([sess.graph.get_tensor_by_name('num_detections:0'),sess.graph.get_tensor_by_name('detection_scores:0'),sess.graph.get_tensor_by_name('detection_boxes:0'),sess.graph.get_tensor_by_name('detection_classes:0')],feed_dict={'image_tensor:0': inp.reshape(1, inp.shape[0], inp.shape[1], 3)})

    		# Visualize detected bounding boxes.
            num_detections = int(out[0][0])
            for i in range(num_detections):
                classId = int(out[3][0][i])
                score = float(out[1][0][i])
                bbox = [float(v) for v in out[2][0][i]]
                if score > 0.3:
                    x = bbox[1] * cols
                    y = bbox[0] * rows
                    right = bbox[3] * cols
                    bottom = bbox[2] * rows
                    height = int(y) - int(bottom)
                    width = int(right) - int(x)
                    ratio = width / height
                    print ('Input - {}, Number of detection - {}, Detection - {}, Class ID - {}, Score - {}, Height - {}, Width - {}, Ratio - {}, Output - {}'.format (image_path, str(num_detections), str( i),str(classId),str(score),height, width, ratio,out_image_path))
                    data = set[{"input":image_path, "num of detection": str(num_detections) , "detection": str(i), "Class": str(classId), "score": str(score) , "height": height, "width": width, "ratio":ratio , "output":out_image_path}]
                    print (data)
                    cv.rectangle(img, (int(x), int(y)), (int(right), int(bottom)), (125, 255, 51), thickness=2)
                    text = "Coke" + str(i)
                    cv.putText(img, text, (int(x),  int(y)), cv.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), lineType=cv.LINE_AA) 
            cv.imwrite(out_image_path,img)
            
            return data
    
    
        
    
    
    